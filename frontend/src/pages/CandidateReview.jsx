...
const CandidateReview = () => {
  ...
  const handleStatusChange = async (candidateId, electionId, newStatus) => {
    ...
    try {
      const res = await fetch(
        `/api/committee/candidates/${candidateId}/elections/${electionId}/status`,
        {
          method: 'PATCH',
          headers: {
  ...
  const handleAcceptAll = (candidate) => {
    if (!window.confirm(`Alle Bewerbungen für ${candidate.lastname} genehmigen?`)) {
      return;
    }
              ...
              <div className="w-full md:w-1/4 flex flex-col items-center text-center border-b md:border-b-0 md:border-r border-gray-100 pb-4 md:pb-0 md:pr-4">
                <div className="w-24 h-24 rounded-full bg-gray-200 mb-3 overflow-hidden flex items-center justify-center text-2xl text-gray-400 font-bold">
                  {candidate.image ? (
                    ...
                    <span>{candidate.lastname[0]}</span>
                  )}
                </div>
                ...
                <div className="space-y-3">
                  {candidate.elections.map((election) => (
                    <div
                      ...
                      className="flex flex-col sm:flex-row sm:items-center justify-between bg-gray-50 p-3 rounded-lg border border-gray-100"
                    >
                      {/* Wahl Info */}
                      ...
                      <div className="flex gap-2">
                        ...
                        <button
                          onClick={() => {
                            // Toggle-Logik
                            const newStatus =
                              election.status === 'ACCEPTED' ? 'PENDING' : 'ACCEPTED';
                        ...
                        <button
                          onClick={() => {
                            // Toggle-Logik
                            const newStatus =
                              election.status === 'REJECTED' ? 'PENDING' : 'REJECTED';
                            handleStatusChange(candidate.id, election.electionId, newStatus);
                          }}
                          className={`p-2 rounded transition-colors border ${
                            election.status === 'REJECTED'
