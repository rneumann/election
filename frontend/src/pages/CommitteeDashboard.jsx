...
const CommitteeDashboard = () => {
  ...
  const ElectionCard = ({ election, statusColor }) => (
    <div 
      onClick={() => handleElectionClick(election)}
      className="bg-white p-6 rounded-lg shadow-sm border border-gray-200 hover:shadow-md hover:border-brand-primary cursor-pointer transition-all group"
    >
      <div className="flex justify-between items-start mb-2">
        <span className={`text-xs font-bold px-2 py-1 rounded-full ${statusColor}`}>
          ID: {election.id}
        </span>
        <svg className="w-5 h-5 text-gray-400 group-hover:text-brand-primary" fill="none" viewBox="0 0 24 24" stroke="currentColor">
          ...
          {elections.current.length === 0 ? (
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
              {elections.current.map(election => (
                <ElectionCard key={election.id} election={election} statusColor="bg-green-100 text-green-800" />
              ))}
            </div>
          )}
        </section>
          ...
          {elections.future.length === 0 ? (
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
              {elections.future.map(election => (
                <ElectionCard key={election.id} election={election} statusColor="bg-blue-100 text-blue-800" />
              ))}
            </div>
          )}
        </section>
